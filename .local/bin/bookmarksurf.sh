...
while true; do 
	...
	case $bookmark in
		...
		DuckDuckGo) choice=$(printf "" | dmenu -m $mon -h 40 -p " DuckDuckGo") ; [ -n "$choice" ] && link="https://duckduckgo.com/?q=""$choice" || exit ; break ;; 
...
[ -n "$choice" ] || [ -n "$idx" ] && $newbrowser "$link" && exit || 
	curl -s --head  --request --fail "$link" && $newbrowser "$link" || $newbrowser "https://searx.bar/search?q=""$link"
